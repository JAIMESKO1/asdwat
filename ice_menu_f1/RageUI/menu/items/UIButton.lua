...
local SettingsButton = {
    ...
    Text = { X = 5, Y = 3, Scale = 0.25 }, -- Modifié X de -5 à 5 pour un meilleur alignement
...
function RageUI.Button(Label, Description, Style, Enabled, Action, Submenu)
    ...
    if CurrentMenu ~= nil and CurrentMenu() then
        ...
        if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
            ...
            if Active then 
                RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + 20 + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 55, SettingsButton.SelectedSprite.Height - 1, 20, 55, 107,125)
            end
            ...
            if Enabled then
                ...
                -- Main Text (Modified for better left alignment)
                RenderText(Label, 
                    CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset + 15, -- Reduced offset from 28 to 15
                    20 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 
                    0, -- Changed from 8 to 0 for left alignment
                ...
                -- Disabled Text (Modified for better left alignment)
                RenderText(Label, 
                    CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset + 15, -- Reduced offset
                    20 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 
                    0, -- Changed from 8 to 0 for left alignment
                    SettingsButton.Text.Scale, 
                    R_ITEM_BUTTON, 
